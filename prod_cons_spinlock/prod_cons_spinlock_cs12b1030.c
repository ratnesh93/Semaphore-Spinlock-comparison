...
int producer_thread_fn(void *n){
   ...
   //   printk("\nthe sub time by taken by producer:  %ld nanoseconds",(APtime.tv_nsec - bPtime.tv_nsec));
...
int consumer_thread_fn(void *n){
    ...
    for(k=0;k<cntc;k++){
      ...
      //printk("\nthe sub time by taken by consumer:  %ld nanoseconds",(ACtime.tv_nsec - bCtime.tv_nsec));
...
static int kernel_init(void)
{
    ...
    for(k=0;k<nc;k++){
        int *t;
        t=&i;
        consumer[k] = kthread_run(&consumer_thread_fn,(void*)t,"consumer");
    }
